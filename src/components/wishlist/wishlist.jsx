...
import Breadcrumbs from '../Breadcums.jsx'

function Wishlist() {
    // const { wish, handlerAddToWish } = useContext(MainContext);
    // Parse the cart from localStorage
    const MainDetails = JSON.parse(localStorage.getItem('wish')) || [];

    // API fetching
    // const [Data, setData] = useState([]);
    const [wislistData, SetwislistData] = useState()

    // API disabled: wishlist syncing to remote server removed
    ...
    useEffect(() => {
        // Replace remote call with local wishlist data only
        SetwislistData(MainDetails);
    }, [MainDetails]);
                                            ...
                                            <li key={v.id}>
                                                <div className="col-sm-6 col-lg-4" key={i}>
                                                    ...
                                                    <div className="card shadow h-100" >
                                                        <img src={v.thumbnail} className="card-img-top" alt="course image" onError={(e) => { e.target.src = "/assets/images/default.jpeg";}}/>
                                                        <div className="card-body pb-0">
                                                            <div className="d-flex justify-content-between mb-2">
                                                                <a href="#" className="badge bg-success bg-opacity-10 text-success">{v.category}</a>
                                                                <a href="#" className="text-danger"><i className="fas fa-heart"></i></a>
                                                            </div>
                                                            <h5 className="card-title fw-normal"><a href="#">{v.title}</a></h5>
